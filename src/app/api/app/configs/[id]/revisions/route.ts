...
const requestSchema = z.object({
  config: lookConfigDocumentSchemaV1,
});
...
export async function POST(
  ...
  const nextConfig = parsedBody.data.config;
  if (
    nextConfig.lookSlug !== configRow.lookVersion.look.slug ||
    nextConfig.lookVersion !== configRow.lookVersion.version
  ) {
    return NextResponse.json({ error: 'Config look mismatch' }, { status: 400 });
  }
  ...
  const revision = await prisma.$transaction(async (db) => {
    ...
    await db.auditLog.create({
      data: {
        actorUserId: user.id,
        action: 'lookConfig.revision.created',
